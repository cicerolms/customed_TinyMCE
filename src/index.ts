import {
  ensureLegacyEditorGlobals,
  LEGACY_EDITOR_VERSION,
  LEGACY_INSERT_MENU_ITEMS,
  LEGACY_PLUGINS,
  LEGACY_TOOLBAR_1,
  LEGACY_WORDPRESS_ADV_HIDDEN,
  legacyPluginSources,
  legacyTinyMceBaseUrl,
  markLegacyActiveEditor,
  resolveLegacyEditorWrap,
  type LegacyEditorHostWindow,
  type LegacyTinyMce,
...
type HtmxApi = {
  process?: (node: ParentNode) => void;
};

type WindowWithEditor = LegacyEditorHostWindow & {
  __i18n?: I18nApi;
  htmx?: HtmxApi;
  __classicEditorAutoBootDone?: boolean;
};

type TinyMceInitConfig = Record<string, unknown>;
...
function getEndpoint(url: string | null | undefined, fallback: string): string {
...
async function loadStyleProfile(profileUrl: string): Promise<EditorStyleProfile> {
  ...
  if (typeof (profile as Record<string, unknown>).bodyClass === "string") {
    next.bodyClass = (profile as Record<string, unknown>).bodyClass as string;
  }
...
function resolveReviewBodyClass(profile: EditorStyleProfile): string {
...
function ensureEditorFragmentHost(target: HTMLElement, textarea: HTMLTextAreaElement, className: string): HTMLElement {
...
function ensureToolbarHost(target: HTMLElement, fragment: HTMLElement): HTMLElement {
  ...
  const shellWrap =
    fragment.closest(".classic-editor-shell-wrap, .wp-editor-wrap") ||
...
function ensureElementId(element: HTMLElement, fallbackId: string): string {
  if (!element.id) {
...
async function loadScript(url: string): Promise<void> {
  ...
  (window as WindowWithEditor).eval(`${code}\n//# sourceURL=${url}`);
}

async function waitForLegacyTinyMce(assetBaseUrl: string): Promise<LegacyTinyMce> {
...
async function loadLegacyPluginScripts(tinymce: LegacyTinyMce, assetBaseUrl: string): Promise<void> {
...
function registerLegacyEditorI18n(tinymce: LegacyTinyMce, lang: string): string {
...
function localizeLegacyMenubar(editor: LegacyTinyMce, i18n?: ClassicEditorI18nConfig | null): void {
  ...
  const nodes = Array.from<Element>(container.querySelectorAll(".mce-menubar .mce-menubtn span"));
...
function selectionUrl(editor: LegacyTinyMce): string {
...
function openLinkCardDialog(editor: LegacyTinyMce, assetBaseUrl: string): void {
...
function insertAtCaret(textarea: HTMLTextAreaElement, value: string): void {
...
async function saveEditorContent(form: HTMLFormElement, statusNode: HTMLElement, saveUrl: string): Promise<BootstrapStatus> {
...
async function loadLatestPost(confirmUrl: string, form: HTMLFormElement, statusNode: HTMLElement): Promise<BootstrapStatus> {
  ...
  const post = responseBody.post as Record<string, string>;
...
function openMediaModal(backdrop: HTMLElement, mediaUrl: string): void {
  if (!mediaUrl) return;
  fetch(mediaUrl, {
    ...
    const htmx = (window as WindowWithEditor).htmx;
...
function closeMediaModal(backdrop: HTMLElement): void {
  backdrop.classList.add("hidden");
  backdrop.innerHTML = "";
  backdrop.setAttribute("aria-hidden", "true");
}

function bindMediaDelegates(backdrop: HTMLElement): void {
...
export async function createClassicEditor(config: ClassicEditorConfig): Promise<ClassicEditorInstance> {
  ...
  const profileStack: EditorStyleProfile[] = [{ ...DEFAULT_EDITOR_STYLE_PROFILE }];
  ...
  const resolvedProfile = profileStack.reduce((current, next) => mergeProfiles(current, next), {} as EditorStyleProfile);
  ...
  const selectCodeRange = (start: number, end: number) => {
    textarea.focus();
  ...
  const isWholeWordMatch = (source: string, start: number, length: number) => {
  ...
  const findInCode = (
    ...
    const findCandidate = (startIndex: number) => {
      if (direction === "next") {
  ...
  const selectionMatchesQuery = (query: string, matchCase: boolean, wholeWord: boolean) => {
  ...
  const replaceCurrentSelection = (query: string, replacement: string, matchCase: boolean, wholeWord: boolean) => {
  ...
  const replaceAllInCode = (query: string, replacement: string, matchCase: boolean, wholeWord: boolean) => {
    if (!query) return 0;
  ...
  const openCodeSearchDialog = (instance: LegacyTinyMce) => {
    ...
    dialog = instance?.windowManager?.open?.({
      ...
      onsubmit(api: any) {
        ...
        const found = findCodeQuery(
          query,
          Boolean(api?.data?.codeSearchMatchCase),
          "next",
          Boolean(api?.data?.codeSearchWholeWords),
        );
        if (!found) {
          instance?.windowManager?.alert?.(notFoundMessage);
        }
  ...
  const openSearchReplace = (instance: LegacyTinyMce) => {
  ...
  const isSearchReplaceTrigger = (eventTarget: EventTarget | null): boolean => {
  ...
  const handleCodeSearchShortcut = (event: KeyboardEvent) => {
    if (!isCodeSurfaceActive()) return;
  ...
  const getLegacyConfig = (): TinyMceInitConfig => ({
    target: fragment,
    inline: true,
    hidden_input: false,
    ...
    setup(instance: LegacyTinyMce) {
      ...
      const registerInlineSourceEntry = (name: string) => {
      ...
      instance.on("focus", () => {
        markLegacyActiveEditor(window as WindowWithEditor, editorId);
      });
  ...
  const ensureVisualEditor = async (): Promise<LegacyTinyMce | null> => {
    if (editor && !editor.removed) {
      return editor;
    }
  ...
  const switchMode = async (nextMode: "visual" | "code"): Promise<void> => {
...
export async function bootstrapClassicEditor(config: SharedEditorBootstrapConfig = {}): Promise<SharedEditorState | null> {
  if (typeof document === "undefined") {
    return null;
  }
...
if (typeof window !== "undefined" && typeof document !== "undefined" && !(window as WindowWithEditor).__classicEditorAutoBootDone) {
  (window as WindowWithEditor).__classicEditorAutoBootDone = true;
  const boot = async () => {
    if (!document.querySelector("[data-classic-editor]")) return;
